...
object SphereImageStore {
    ...
    private const val ALBUM = "Nalabo360"
    ...
    /**
     * A stitched sphere on its way to the user is a [StitchedSphere]: a
     * GPano-tagged JPEG in the cache plus the dimensions written into that
     * metadata. The record itself lives in common code so the result screen can
     * be shared; this store only ever builds one from a `File`.
     */
    ...
    fun stampCaptureOrientation(
        ...
        try {
            ExifInterface(file).apply {
                ...
                setAttribute(ExifInterface.TAG_SOFTWARE, "Nalabo360")
    ...
    fun writeStitchedSphere(
        ...
        gpano: GPanoMetadata = GPanoMetadata.forFullPano(image.width, image.height),
    ): StitchedSphere {
        ...
        try {
            // The platform codec turns the shared pixel buffer into the one JPEG
            // everything downstream - preview, export, share - works from.
            val jpeg = platformImageCodec().encodeJpeg(image, quality)
                ?: throw IOException("Could not encode the stitched sphere")
            FileOutputStream(temp).use { out -> out.write(jpeg) }
            stampSphereDescription(temp)
            GPanoXmpInjector.inject(temp.absolutePath.toPath(), gpano)
                .onFailure { error ->
