...
object SphereProjection {
    ...
    fun cameraFrame(orientation: OrientationData): CameraFrame {
        val yaw = toRadiansSafe(orientation.yawDegrees.toDouble())
        val elevation = toRadiansSafe(orientation.elevationDegrees().toDouble())
        val roll = toRadiansSafe(orientation.rollDegrees.toDouble())
...
class CameraFrame internal constructor(
    ...
    fun project(target: SphereTarget): TargetView {
        ...
        return TargetView(
            ...
            z = z.toFloat(),
            angularDistanceDegrees = toDegreesSafe(acos(z.coerceIn(-1.0, 1.0))).toFloat(),
        )
    }
}
