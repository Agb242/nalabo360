...
class CameraBasis private constructor(
    ...
    fun toPose(): CameraPose {
        val yaw = toDegreesSafe(atan2(forwardX, forwardY)).toFloat()
        val elevation = toDegreesSafe(asin(forwardZ.coerceIn(-1.0, 1.0))).toFloat()
        // Rebuild the unrolled pair for this yaw/elevation and measure how far
        // the actual up/right pair has rolled about forward.
        val yawR = toRadiansSafe(yaw.toDouble())
        val elevationR = toRadiansSafe(elevation.toDouble())
        val sinYaw = sin(yawR)
        val cosYaw = cos(yawR)
    ...
    companion object {
        ...
        fun of(pose: CameraPose): CameraBasis {
            ...
            val yaw = toRadiansSafe(pose.yawDegrees.toDouble())
            val elevation = toRadiansSafe(pose.elevationDegrees.toDouble())
            val roll = toRadiansSafe(pose.rollDegrees.toDouble())
...
data class FrameIntrinsics(
    ...
    companion object {
        ...
        private fun tanHalf(degrees: Float): Double = tan(toRadiansSafe(degrees / 2.0))
    }
}
...
object Equirectangular {
    ...
    fun direction(longitudeDegrees: Double, latitudeDegrees: Double): DoubleArray {
        val lon = toRadiansSafe(longitudeDegrees)
        val lat = toRadiansSafe(latitudeDegrees)
        val cosLat = cos(lat)
        return doubleArrayOf(sin(lon) * cosLat, cos(lon) * cosLat, sin(lat))
    }

    /** Longitude of a world direction, -180°..180°. */
    fun longitudeOf(x: Double, y: Double): Double = toDegreesSafe(atan2(x, y))

    /** Latitude of a *unit* world direction, -90°..90°. */
    fun latitudeOf(z: Double): Double = toDegreesSafe(asin(z.coerceIn(-1.0, 1.0)))
}
...
object FrameFootprint {
    ...
    private fun ceilToInt(value: Double): Int = ceil(value).toInt()
}
...
internal fun pivotDirection(
    ...
    val distance = -half + sqrt(max(half * half - pivotRatio * pivotRatio + 1.0, 0.0))
    ...
    val length = sqrt(px * px + py * py + pz * pz)
...
private fun tanOf(degrees: Float): Double =
    if (degrees >= 90f) Double.MAX_VALUE else tan(toRadiansSafe(degrees.toDouble()))
