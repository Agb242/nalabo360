 ...
 *
 * Everything here runs on plain buffers ([RgbImage], flat arrays), exactly like
 * the rest of the shared pipeline — [SeamSolver], [SeamWeights] and
 * [SeamFeather] were already pure Kotlin.
 */
internal object SeamFinder {
    ...
    fun computeSeams(
        ...
        // Each frame's pixels are read through one flat RGB buffer rather than
        // a per-sample lookup into a large image: the projection back into the
        // frame runs millions of times and a byte buffer indexed by hand is far
        // cheaper than bounds-checked image access per pixel. The copy comes
        // from a small downsampled version so holding every frame at once costs
        // a few megabytes, not the hundreds a full-resolution working set would.
    ...
    /**
     * Downsamples [image] so its long edge is at most [WORK_LONG_EDGE], as an
     * area average like the old `Imgproc.resize(…, INTER_AREA)` call.
     *
     * Channel order note: the original Android pipeline stored mats as BGR and
     * read them back BGR here; only colour *distances* and means are consumed
     * downstream, which are invariant under a global channel permutation, so
     * reading straight RGB is equivalent.
     */
    private fun downsampleAndRead(image: RgbImage): WorkingFrame {
        val sourceWidth = image.width
        val sourceHeight = image.height
        val longest = max(sourceWidth, sourceHeight)
        val scale = longest / WORK_LONG_EDGE.toDouble()
        return if (scale <= 1.0) {
            WorkingFrame(image, sourceWidth, sourceHeight, scale)
        } else {
            val width = max(1, (sourceWidth / scale).roundToInt())
            val height = max(1, (sourceHeight / scale).roundToInt())
            WorkingFrame(ImageMath.resizeArea(image, width, height), width, height, scale)
        }
    }
    ...
    private fun sampleInto(
        ...
        val index = (rowInt * bytes.width + col) * 3
        out[0] = (bytes.bytes[index].toInt() and 0xff).toDouble()
        out[1] = (bytes.bytes[index + 1].toInt() and 0xff).toDouble()
        out[2] = (bytes.bytes[index + 2].toInt() and 0xff).toDouble()
        out[3] = column
        out[4] = row
        return true
    }
